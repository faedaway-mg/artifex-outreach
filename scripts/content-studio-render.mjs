...
// Reads the durable job file (.data/content-studio/jobs/<jobId>.json), renders the Field Notes scene
// via the real engine (headless Chrome + ffmpeg), embeds the generated thumbnail as frame zero, and
// writes progress + the final output back into the job file. Two modes:
//   • reuse-approved-audio  → copy the approved final audio stream byte-for-byte (native regen)
//   • uploaded-vo           → sync to the uploaded voiceover and build a fresh cue+voice mix
...
async function main() {
  ...
  if (!cfg) throw new Error(`no scene wired for #${note}`);
  ...
  if (!existsSync(thumbPath)) throw new Error("thumbnail missing: " + thumbPath);
  ...
  // Resolve the voiceover and the synced timeline.
  ...
  // Render frames (native 1080×1920 viewport → no white bar), streaming progress into the job file.
  ...
  // Thumbnail-first: frame zero = the generated cover.
  ...
  if (job.mode === "reuse-approved-audio") {
    ...
    // Build a fresh cue-only stem + voice mix for the uploaded VO (no atmosphere; VOICE dominant).
  ...
  patchJob({
    ...
    audioLabel: job.audioLabel || audioNote,
  });
