...
async function main() {
  ...
  try {
    await page.goto(created.url, { waitUntil: "domcontentloaded", timeout: 60000 });
    const fill = async (sel: string, val: string) => { const el = page.locator(sel).first(); if (await el.count()) { await el.fill(val).catch(() => {}); } };
    await fill('input[name="email"]', "sandbox@artifexlabs.tech");
    await fill('input[name="cardNumber"]', "4242424242424242");
    await fill('input[name="cardExpiry"]', "12 / 34");
    await fill('input[name="cardCvc"]', "123");
    await fill('input[name="billingName"]', "Sandbox Tester");
    await fill('input[name="billingPostalCode"]', "42424");
    // Country/address may be required; try a postal-only or full-address form.
    await page.locator('.SubmitButton, button[type="submit"]').first().click({ timeout: 15000 }).catch(() => {});
    await page.waitForURL(/\/offer\/.+\/success/, { timeout: 60000 }).catch(() => {});
    check("hosted checkout submitted + redirected to success page", /\/success/.test(page.url()), page.url().slice(0, 80));
  } catch (e) {
    check("hosted checkout automation", false, e instanceof Error ? e.message.slice(0, 80) : "err");
