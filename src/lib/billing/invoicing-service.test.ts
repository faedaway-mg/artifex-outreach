...
describe("prepareMilestoneInvoice", () => {
  it("creates a draft deposit invoice bound to the exact version + issuer, idempotently", async () => {
    ...
    const r = await prepareMilestoneInvoice(ag.id, "deposit", { actor: "jordan" });
    ...
    // idempotent
    const again = await prepareMilestoneInvoice(ag.id, "deposit", { actor: "jordan" });
  ...
  it("refuses a milestone before it is eligible; allows it once acceptance is recorded", async () => {
    const ag = await signedAgreement();
    const blocked = await prepareMilestoneInvoice(ag.id, "balance", { actor: "jordan" });
    expect(blocked.ok).toBe(false);
    expect(blocked.blocked).toBe(true);
    const ok = await prepareMilestoneInvoice(ag.id, "balance", { actor: "jordan", acceptedKeys: ["balance"] });
...
describe("issueMilestoneInvoice (external, gated)", () => {
  it("issues in test mode: finalizes and stores the hosted invoice URL", async () => {
    ...
    const ag = await signedAgreement();
    const prep = await prepareMilestoneInvoice(ag.id, "deposit", { actor: "jordan" });
    const issued = await issueMilestoneInvoice(prep.invoice!.id, { actor: "jordan", requireTestMode: true, fetchImpl: fakeStripe() });
  ...
  it("refuses to issue live when sending is disabled", async () => {
    const ag = await signedAgreement();
    const prep = await prepareMilestoneInvoice(ag.id, "deposit", { actor: "jordan" });
    const r = await issueMilestoneInvoice(prep.invoice!.id, { actor: "jordan", requireTestMode: false });
    expect(r.ok).toBe(false);
