...
export async function storeFrozenReviewPdf(leadId: string, version: number, pdfBase64: string): Promise<string> {
  const key = frozenReviewPdfKey(leadId, version);
  const bytes = Buffer.from(pdfBase64, "base64");
  const sha256 = sha256OfBytes(bytes);
  const existing = await getSignedArtifactBlob(key);
  if (existing) {
    if (existing.sha256 !== sha256) {
      throw new Error(`frozen Quick Review artifact ${key} already exists with a different SHA (immutable version collision)`);
    }
    return existing.sha256; // idempotent
  }
...
export async function loadFrozenReviewPdf(leadId: string, version: number): Promise<FrozenReviewPdf | null> {
  const blob = await getSignedArtifactBlob(frozenReviewPdfKey(leadId, version));
  if (!blob) return null;
  return {
    pdfBase64: Buffer.from(blob.data).toString("base64"),
    sha256: blob.sha256,
