...
describe("Quick-Cash is exposed in the primary sidebar", () => {
  it("the shell nav exposes Quick-Cash (now the default home route)", () => {
    // NAV destinations live in the shared nav-items module (consumed by the Shell sidebar).
    const src = readFileSync(path.join(process.cwd(), "src/components/nav-items.ts"), "utf8");
