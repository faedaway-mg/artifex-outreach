...
function job(p: Partial<RenderJob>): RenderJob {
  ...
  return {
    ...
    status: p.status ?? "queued", progress: p.progress ?? 0, stage: p.stage ?? "", mode: p.mode ?? "uploaded-vo", audioKind: p.audioKind ?? "uploaded",
    audioFile: p.audioFile ?? null, audioKey: p.audioKey ?? null, audioSha: p.audioSha ?? null, audioLabel: null, outputFile: p.outputFile ?? null, outputRel: p.outputRel ?? null, outputKey: p.outputKey ?? null, posterKey: p.posterKey ?? null,
