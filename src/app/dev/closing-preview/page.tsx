...
function row(over: Partial<ScheduleRow>): ScheduleRow {
  return { key: "deposit", label: "Deposit", amountCents: 725_000, trigger: "on_signature", eligible: true, invoiceState: "not-created", invoiceId: null, hostedInvoiceUrl: null, disputeStatus: "none", amountRefundedCents: 0, ...over };
}
const money0 = { currency: "usd", invoicedCents: 0, collectedCents: 0, refundedCents: 0, chargebackLostCents: 0, disputedOpenCents: 0, outstandingCents: 0, payoutStatus: "unverified" as const, bankReceiptStatus: "unverified" as const };
...
const STATES: Record<string, ClosingView> = {
  ...
  pending: base({
    ...
    money: { ...money0, invoicedCents: 725_000, outstandingCents: 725_000 },
    nextAction: { label: "Awaiting deposit payment", kind: "await-external" },
  }),
  refund: base({
    // Deposit was paid then partially refunded; balance paid then dispute LOST (chargeback, not a refund).
    schedule: [row({ invoiceState: "paid", amountRefundedCents: 100_000 }), row({ key: "balance", label: "Final balance", trigger: "on_acceptance", eligible: true, invoiceState: "paid", disputeStatus: "lost" })],
    money: { ...money0, invoicedCents: 1_450_000, collectedCents: 625_000, refundedCents: 100_000, chargebackLostCents: 725_000 },
    nextAction: { label: "Review chargeback loss on Final balance", kind: "await-external", blockerReason: "A dispute was lost (chargeback)." },
  }),
  ...
  long: base({
    ...
    money: { ...money0, invoicedCents: 100_150_000, collectedCents: 12_500_000, outstandingCents: 87_650_000 },
  }),
};
